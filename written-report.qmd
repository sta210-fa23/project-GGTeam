...
tiktok_users <- tiktok|>
  ...
  mutate(followers = followers - mean(followers),
         shares = shares - mean(shares),
         comments = comments - mean(comments),
         plays = plays - mean(plays),
         likes = likes - mean(likes),
         total_videos = total_videos - mean(total_videos))
```
...
m2 <- linear_reg() |>
  set_engine("lm") |>
  fit(log(followers) ~ . - video_length - video_length_bin - user_name, data = tiktok_users)
