...
m1 <- linear_reg() |>
  set_engine("lm") |>
  fit(followers ~ . - likes - user_name, data = tiktok_users)
...
m2 <- linear_reg() |>
  set_engine("lm") |>
  fit(followers ~ . - video_length_bin - user_name, data = tiktok_users)
...
ggplot(data = tiktok_aug, aes(x = video_length, y = .resid)) +
    geom_point() +
    geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
    labs(x = "Fitted values",
         y = "Residuals",
