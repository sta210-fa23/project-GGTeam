...
The dataset comes from the 'top_users_vids.csv' file of this [Github repository](https://github.com/ivantran96/TikTok_famous/tree/main), which was originally collected as part of the DataResolutions's Data Blog project exploring Tiktok's demographics and trending video analytics. 
...
When observing the video length bin variable, the longest video length bin (3) has statistically significant difference from the other two video length bins and a statistically significant interaction term with likes and shares. This shows that not only do long length videos generate the most followers, but long length videos combined with a higher likes and shares indicate a higher follower count as well. This makes sense since viewers will be more invested in long videos (as can be confirmed by positive engagement), encouraging them to follow the creator.

## Discussion + Conclusion

We originally decided to look at TikTok's data and how follower count (a huge driver of engagement) is impacted by other aspects of a user's account. We learned that it is extremely difficult to correctly predict follower count, given our model only captures 17.7% of the variability in the dataset.
