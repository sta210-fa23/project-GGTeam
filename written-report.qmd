...
For our coefficents, we just wish to examine the predictor with the largest effect on user followers, so likes. Hence, for every addition million average likes of a user, we expect the expected to multiply by a factor of about 1.481(exp\[0.3925\]), compared to the baseline with mean number of shares, total_videos, and with an average video length belonging to bin_length1, holding all else constant.
